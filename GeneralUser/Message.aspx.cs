...
namespace LearnSphere_WAPP.GeneralUser
{
    public partial class Message : System.Web.UI.Page
    {
        ...
        {
            // Security Check: Ensure user is logged in and is a General user
            ...
            using (SqlConnection conn = new SqlConnection(connString))
            {
                string query = @"
                    SELECT 
                        c.conversationid,
                        CASE 
                            WHEN c.userid = @UserID THEN (u2.fname + ' ' + u2.lname)
                            ELSE (u1.fname + ' ' + u1.lname)
                        END AS DisplayName,
                        CASE 
                            WHEN c.userid = @UserID THEN u2.ProfileImage
        ...
        {
            if (string.IsNullOrWhiteSpace(txtMessage.Text) || CurrentConversationID == 0) return;
